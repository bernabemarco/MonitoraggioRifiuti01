CREATE VIEW CGesVistaRegistrazioniMetodoAI AS 
   SELECT BT.CodDestinazione, BT.Nome, BT.Ambiente, BT.AnnoInizio, BT.MeseInizio, BT.AnnoFine, BT.MeseFine, BT.DesIntestazione, 
          BR.Numero, BR.ContoVoce, BR.Descrizione, BR.ImportoTotale, TD.Anno, TD.Mese, 
   ...
   FROM CGesAnalisiIndividualeTeste BT INNER JOIN CGesAnalisiIndividualeRighe BR ON BT.Progressivo = BR.RifProgressivo
        LEFT OUTER JOIN CGesTransazioniDistribuite TD ON TD.CodConto = BR.ContoVoce AND TD.Ambiente = BT.Ambiente AND TD.CodDestinazione = BT.CodDestinazione AND 
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[CGesVistaRegistrazioniMetodoAI] TO [Metodo98]
    AS [dbo];
