...
--drop view GEM_VistaDettAnomalieMezziRapportoOfferte
--go


CREATE VIEW [dbo].[GEM_VistaDettAnomalieMezziRapportoOfferte]
AS
SELECT     t.IDCONTRATTO, t.CODCLIENTE, t.CODCLIFAT, t.DATACONTRATTO, t.DATAEFFICACIA, t.DATAPVISITA, t.DURATA, t.FLAG_PROV, t.PAGAMENTO, t.NOTE, t.REFERENTE, 
                      ...
                      dm.CarTesto7, dm.CarTesto8, dm.CarTesto9, dm.CodiceBarre, dm.OMOLOGATO, dm.ANNO, dm.FLAG_INST, dm.STATO, dm.POSQLIK, dm.QUANTITA, dm.TEMPO_MAN, 
                      dm.IMPORTOAT1, dm.IMPORTOAT2, dm.IMPORTOAT3, dm.IMPORTOAT4, dm.IMPORTOAT5, dm.SCONTOAT1, dm.SCONTOAT2, dm.SCONTOAT3, dm.SCONTOAT4, 
                      dm.SCONTOAT5, dm.PROV1AT1, dm.PROV1AT2, dm.PROV1AT3, dm.PROV1AT4, dm.PROV1AT5, dm.PROV2AT1, dm.PROV2AT2, dm.PROV2AT3, dm.PROV2AT4, 
                      dm.PROV2AT5, dm.PROV3AT1, dm.PROV3AT2, dm.PROV3AT3, dm.PROV3AT4, dm.PROV3AT5, dm.DATA_REV, dm.DATA_PREV, dm.DATA_COLL, dm.DATA_PCOLL, 
                      dm.StatoFuoriSede, dm.StatoAttivita, dm.FLAGAT1, dm.FLAGAT2, dm.FLAGAT3, dm.FLAGAT4, dm.FLAGAT5, dm.FLAGRICARICA, dm.IDRAPPORTO, 
                      rp.STATO AS STATO_rp, rp.DA_FATT, rp.ELIMINATO AS ELIMINATO_rp, rp.TECNICO_EFF, agetr.DSCAGENTE AS DescTecnicoEff, exAgTecnEff.TipoAgente, rp.DATA_EFF, 
                      rp.TECNICO_PREV, rp.TIPOLOGIARAPPORTO, rp.TIPO_RAPPORTO, a.DSCCONTO1 AS RagSocClienteFatt, vCli.DSCCONTO1 AS RagSocCliente, vCli.CODSETTORE, 
                      rp.NOTE_INTERNE AS Note_interneRP, vCli.CODZONA, dm.AnnotazioniTecnica, DM_Anomalie.IdRigaAnomalia, DM_Anomalie.idAnomalia, 
                      ...
                      DM_Anomalie.DATAMODIFICA, TRifOfferta.idOffertaCRM, TRifOfferta.ProtocolloOfferta, TRifOfferta.dataEmissioneOfferta, TRifOfferta.dataRichiestaOfferta, 
                      TRifOfferta.TotaleNettoComplessivo, TRifOfferta.TotaleNettoDiTrattativa, TRifOfferta.IdAsa, TRifOfferta.IdSottoAsa, TRifOfferta.ProtocolloPadre, 
                      TRifOfferta.NumRevisione, TRifOfferta.StatoOfferta, TRifOfferta.Esito, TRifOfferta.TipoOfferta, TRifOfferta.AutoreDocumento, TRifOfferta.TrattativaApprovataDa, 
                      TRifOfferta.TotPercScontoTrattativa, TRifOfferta.OggettoOfferta, TRifOfferta.IdDivisione
FROM         ANAGRAFICAAGENTI AS age2 RIGHT OUTER JOIN
                      ANAGRAFICAAGENTI AS age1 RIGHT OUTER JOIN
                      ANAGRAFICACF AS a RIGHT OUTER JOIN
                      CRM_ALD_TabRiffOfferta AS TRifOfferta RIGHT OUTER JOIN
                      CRM_ALD_TabRiffOffertaIdRapportoGemma AS TRifOffIdRappGemma ON TRifOfferta.idOffertaCRM = TRifOffIdRappGemma.idOffertaCRM RIGHT OUTER JOIN
                      GEM_T_TAB_ANOMALIE AS TTabAnomalie INNER JOIN
                      GEM_SEZIONECONTRATTORAPPORTIMEZZI_Anomalie AS DM_Anomalie ON TTabAnomalie.idAnomalia = DM_Anomalie.idAnomalia INNER JOIN
                      GEM_SEZIONECONTRATTORAPPORTIMEZZI AS dm ON DM_Anomalie.IDCONTRATTO = dm.IDCONTRATTO AND 
                      ...
                      GEM_SEZIONECONTRATTORAPPORTI AS rp ON sc.IDCONTRATTO = rp.IDCONTRATTO AND sc.SEZIONECONTRATTO = rp.SEZIONECONTRATTO ON 
                      dm.SEZIONECONTRATTO = rp.SEZIONECONTRATTO AND dm.IDCONTRATTO = rp.IDCONTRATTO AND dm.IDRAPPORTO = rp.IDRAPPORTO INNER JOIN
                      ANAGRAFICAARTICOLI AS AnaArt ON dm.CodMezzo = AnaArt.CODICE ON TRifOffIdRappGemma.idRapporto = rp.IDRAPPORTO LEFT OUTER JOIN
                      GEM_T_TAB_STATOAnomalie AS TStAnomalia ON DM_Anomalie.idStatoAnomalia = TStAnomalia.CODICE LEFT OUTER JOIN
                      GEM_T_TAB_LivelloRischio AS TTabLivelloRischio ON TTabAnomalie.idLivelloRischio = TTabLivelloRischio.idLivelloRischio ON 
                      a.CODCONTO = t.CODCLIFAT LEFT OUTER JOIN
                      VISTACLIENTI AS vCli ON t.CODCLIENTE = vCli.CODCONTO ON age1.CODAGENTE = t.CodAgente1 ON age2.CODAGENTE = t.CodAgente2 LEFT OUTER JOIN
                      ...
                      ANAGRAFICAAGENTI AS agetr ON rp.TECNICO_EFF = agetr.CODAGENTE
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[GEM_VistaDettAnomalieMezziRapportoOfferte] TO [Metodo98]
    AS [dbo];
