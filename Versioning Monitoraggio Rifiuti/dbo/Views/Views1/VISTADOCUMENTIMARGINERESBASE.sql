...
CREATE VIEW VISTADOCUMENTIMARGINERESBASE AS
SELECT TD.PROGRESSIVO AS IDTESTA, TD.ESERCIZIO,TD.DATARIFDOC,
    ...
    (SELECT FORMULAFC FROM VISTAFATTORICONVERSIONE WHERE((CODART=RD.CODART) AND (UMJOIN=VISTAARTICOLIUMBASE.UM1) AND (UMRIC=(SELECT UM FROM STORICOPREZZIARTICOLO WHERE ((CODARTICOLO=RD.CODART) AND (TIPOPREZZO='F')AND (ULTIMO=1)))))) AS CONVPRZUNITSTORICO,
    ...
    (SELECT UM FROM STORICOPREZZIARTICOLO WHERE ((CODARTICOLO=RD.CODART) AND (TIPOPREZZO='F')AND (DATA=(SELECT MAX(DATA) FROM STORICOPREZZIARTICOLO WHERE ((TIPOPREZZO='F') AND (CODARTICOLO=RD.CODART) AND (DATA<=TD.DATADOC)))))) AS UMUNITSTOR,
    (select FORMULAFC from VISTAFATTORICONVERSIONE where((CODART=RD.CODART) and (UMJOIN=VISTAARTICOLIUMBASE.UM1) and(UMRIC=(select UM from STORICOPREZZIARTICOLO where((CODARTICOLO=RD.CODART) and(TIPOPREZZO='F') and(DATA=(select MAX(DATA) from STORICOPREZZIARTICOLO where((TIPOPREZZO='F') and(CODARTICOLO=RD.CODART) and(DATA<=TD.DATADOC))))))))) as CONVPRZUNIT,
    ...
    RD.RIFRELAZIONECF,RD.UBICAZIONE,RD.UBICAZIONECOLL
FROM
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[VISTADOCUMENTIMARGINERESBASE] TO [Metodo98]
    AS [dbo];
