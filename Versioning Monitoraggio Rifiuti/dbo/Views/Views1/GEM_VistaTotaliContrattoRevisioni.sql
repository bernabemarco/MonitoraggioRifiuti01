...
CREATE VIEW [dbo].[GEM_VistaTotaliContrattoRevisioni]
AS
SELECT     TC.IDCONTRATTO, SCDM.SEZIONECONTRATTO, TC.CODCLIENTE, SC.idTipologMezzo, SC.CodiceMezzo, SCDM.CodMezzo AS Articolo, SC.NrVisiteTot, 
                      SC.IdStrutturaOpPr, SC.Revisione, SCDM.DATA_PREV AS Data_Prev, SUM(SCDM.QUANTITA) AS Quantita, 
(CASE WHEN
                      ...
                      > 0 THEN ((CASE WHEN
                          (SELECT     Primaria
                            ...
                            WHERE      VAM.idTipologMezzo = SC.idTipologMezzo AND VAM.Posizione = 2) = 0 THEN SUM(SCDM.QUANTITA * SCDM.IMPORTOAT2) ELSE 0 END))
                       /
                          (SELECT     isnull(NrOperazioniTotali, 1)
                            FROM          GEM_Vista_T_TabStruttureOperazioniPrimarie VOP
                            WHERE      VOP.idTipologMezzo = SC.idTipologMezzo AND VOP.IdStrutturaOpPr = SC.IdStrutturaOpPr AND VOP.Revisione = SC.Revisione) 
...
(CASE WHEN
                      ...
                      > 0 THEN ((CASE WHEN
                          (SELECT     Primaria
                            FROM          GEM_VistaAttivitaMezzi VAM
                            WHERE      VAM.idTipologMezzo = SC.idTipologMezzo AND VAM.Posizione = 2) = 0 THEN SUM(SCDM.QUANTITA * SCDM.IMPORTOAT2 * ISNULL(SCDM.PROV1AT2,0)/100) ELSE 0 END))
                       /
                          (SELECT     isnull(NrOperazioniTotali, 1)
                            FROM          GEM_Vista_T_TabStruttureOperazioniPrimarie VOP
                            WHERE      VOP.idTipologMezzo = SC.idTipologMezzo AND VOP.IdStrutturaOpPr = SC.IdStrutturaOpPr AND VOP.Revisione = SC.Revisione) 
...
WHERE     (ISNULL(SCDM.ELIMINATO, '0') = '0') AND (CAST(SCDM.DATA_PREV AS decimal(18, 5)) > 0) OR
                      (CAST(SCDM.DATA_PREV AS decimal(18, 5)) > 0) AND (SCDM.ELIMINATO = '')
GROUP BY TC.IDCONTRATTO, TC.CODCLIENTE, SC.idTipologMezzo, SC.CodiceMezzo, SCDM.CodMezzo, SC.NrVisiteTot, SC.IdStrutturaOpPr, SC.Revisione, 
                      SCDM.DATA_PREV, SCDM.SEZIONECONTRATTO

GO
GRANT SELECT
    ON OBJECT::[dbo].[GEM_VistaTotaliContrattoRevisioni] TO [Metodo98]
    AS [dbo];
