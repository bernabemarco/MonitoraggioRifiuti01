...
CREATE VIEW FRCVistaDatiStorici AS 
	SELECT ID, AMBIENTE AS Ambiente, ISNULL((SELECT DESCRIZIONE FROM CGesAmbienti WHERE Codice = FRCDatiStorici.AMBIENTE), '') AS Descrizione_Ambiente, ANNODOC AS Anno, 
		   ...
		   CONTOCDC AS Codice_Centro, ISNULL((SELECT DESCRIZIONE FROM TABCENTRICOSTO WHERE CODICE = FRCDatiStorici.CONTOCDC), '') AS Descrizione_Centro, 
		   CDRMercato AS CDR_Mercato, ISNULL((SELECT DESCRIZIONE FROM TABCENTRICOSTO WHERE CODICE = FRCDatiStorici.CDRMercato), '') AS Descrizione_CDR_Mercato, 
		   CDRProdotto AS CDR_Prodotto, ISNULL((SELECT DESCRIZIONE FROM TABCENTRICOSTO WHERE CODICE = FRCDatiStorici.CDRProdotto), '') AS Descrizione_CDR_Prodotto, RIFCOMMCLI, RIFCOMMRIEP, 
		   MESEDOCUMENTO AS Mese_Competenza, TipoClienteBudget AS Tipo_Cliente_Budget, ANNOCOMPETENZA AS Anno_Competenza
	FROM FRCDatiStorici

GO
GRANT SELECT
    ON OBJECT::[dbo].[FRCVistaDatiStorici] TO [Metodo98]
    AS [dbo];
