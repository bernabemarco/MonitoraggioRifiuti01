...
create VIEW [dbo].[GEM_VistaListiniContrattoMatrice]
as
select 
...
from  dbo.GEM_ListiniContrattiMatrice t left outer join dbo.GEM_ListiniContrattoRigheMatrice r on t.idGListinoContr=r.idGListinoContr
	left outer join dbo.GEM_ListiniContrattoRigheAttivitaMatrice a on r.idGlistinoContr = a.idGListinoContr 
		and r.idRigaListinoC = a.idRigaListinoC
	left outer  join [GEM_VistaListiniContrattiRigheMatrice] vr
	on r.idGListinoContr= vr.idGListinoContr and  r.IdRigaListinoC=vr.IdRigaListinoC


GO
GRANT REFERENCES
    ON OBJECT::[dbo].[GEM_VistaListiniContrattoMatrice] TO [Metodo98]
    AS [dbo];
