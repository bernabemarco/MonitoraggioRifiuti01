...
CREATE VIEW [dbo].[FRCVistaDocumentiVendite] AS 
	SELECT YEAR(TESTEDOCUMENTI.DATADOC) AS ANNODOC, MONTH(TESTEDOCUMENTI.DATADOC) AS MESEDOC, LTRIM(STR(YEAR(TESTEDOCUMENTI.DATADOC))) + CASE WHEN MONTH(TESTEDOCUMENTI.DATADOC) 
		   ...
		   RIGHEDOCUMENTI.UMPREZZO, SUM(RIGHEDOCUMENTI.QTAPREZZO * TESTEDOCUMENTI.SEGNO * (CASE WHEN ANAGRAFICACF.TIPOCONTO = 'C' THEN 1 ELSE -1 END)) AS QTAPREZZO, 
		   RIGHEDOCUMENTI.CODIVA, LEFT(ISNULL(TRATTAMENTOIVA.DESCRIZIONE, ''), 25) AS DESCIVA, ISNULL(TRATTAMENTOIVA.ALIQUOTA, 0) AS ALIQUOTA, ISNULL(TRATTAMENTOIVA.INDETRAIBILITA, 0) AS INDETRAIBILITA, 
	...
	GROUP BY YEAR(TESTEDOCUMENTI.DATADOC), MONTH(TESTEDOCUMENTI.DATADOC), LTRIM(STR(YEAR(TESTEDOCUMENTI.DATADOC))) 
			 ...
			 ELSE LTRIM(STR(MONTH(TESTEDOCUMENTI.DATADOC))) END, ANAGRAFICACF.TIPOCONTO, TESTEDOCUMENTI.CODCLIFOR, 
			 ANAGRAFICACF.DSCCONTO1, RIGHT(ANAGRAFICACF.DSCCONTO1, 150) + ' - ' + RIGHT(TESTEDOCUMENTI.CODCLIFOR, 50), 
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FRCVistaDocumentiVendite] TO [Metodo98]
    AS [dbo];
