...
CREATE VIEW [dbo].[GEM_VistaContratti] AS
SELECT     t.RIFCOMMCLI, t.IDCONTRATTO, t.CODCLIENTE, t.CODCLIFAT, t.DATACONTRATTO, t.DATAEFFICACIA, t.DATAPVISITA, t.DURATA, t.FLAG_PROV, t.PAGAMENTO, t.NOTE, 
                      ...
                      CASE isnull(sc.coddest, 0) WHEN 0 THEN
                          ...
                          (SELECT     i.CAP
                            FROM          destinazionidiverse i
                            WHERE      i.CODCONTO = t .CODCLIENTE AND i.CODICE = sc.CODDEST) END AS SedeCAP, CASE isnull(coddest, 0) WHEN 0 THEN
                          (SELECT     a.CodSettoreGiriVisite
                            FROM          dbo.GEM_TESTACONTRATTO cr INNER JOIN
                                                   EXTRACLIENTI a ON cr.IDCONTRATTO = t .IDCONTRATTO AND cr.CODCLIENTE = a.codconto) ELSE
                          (SELECT     i.CodSettoreGiriVisite
                            FROM          destinazionidiverse i
                            WHERE      i.CODCONTO = t .CODCLIENTE AND i.CODICE = sc.CODDEST) END AS SedeGiriVisite, a.TELEFONO, a.FAX, a.TELEX, ISNULL(sc.TEL_REF, 
                      '') AS SEZ_Tel_Ref, ISNULL(sc.EMAIL_RESPONSABILE, '') AS SEZ_Email_Responsabile, ISNULL(t.IdDivisione, 0) AS IdDivisione, 
                      ISNULL(ATD.DscDivisione, '') AS DscDivisione, sc.IdDivisione AS idDivisioneSez, ATDSez.DscDivisione AS DscDivisioneSez, 
					  sc.NrVisiteTot,
					  T.IdTipoCli, t.FLG_ApplAumentoIstat
...
FROM         GEM_TESTACONTRATTO AS t INNER JOIN
                      ...
                      GEM_Vista_TipologMezzo AS tm ON sc.idTipologMezzo = tm.idTipologMezzo LEFT OUTER JOIN
                      Ald_TabDivisioni AS ATDSez ON sc.IdDivisione = ATDSez.IdDivisione LEFT OUTER JOIN
                      ANAGRAFICAAGENTI AS agetr ON sc.CodTecnicoRisorsa = agetr.CODAGENTE LEFT OUTER JOIN
                      Ald_TabDivisioni AS ATD ON ATD.IdDivisione = t.IdDivisione



GO
GRANT SELECT
    ON OBJECT::[dbo].[GEM_VistaContratti] TO [Metodo98]
    AS [dbo];
