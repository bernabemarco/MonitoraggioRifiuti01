...
CREATE VIEW [dbo].[ALD_Vista_UffAcqAnagraficoClientiRiservati]
AS
SELECT     TABDITTE.DITTA, AnaCRiserv.ESERCIZIO, (CASE WHEN LEFT(isnull(anaC.PARTITAIVA, '00000000000'), 11) 
                      ...
                      anaC.PARTITAIVA, anaC.CODFISCALE, AnaCRiserv.CODPAG, AnaCRiserv.UTENTEMODIFICA, AnaCRiserv.DATAMODIFICA, SUM(ISNULL(RIGHEIVA.IMPONIBILEEURO, 
                      0)) AS Fatturato, EXC.IdTrust, AnaCRiserv.CODAGENTE1,
                          ...
                          (SELECT     SUM(IMPORTOSCEURO) AS Expr1
                            FROM          TABSCADENZE AS TABSCADENZE_1
                            WHERE      (CODCLIFOR = anaC.CODCONTO) AND (ESITO IN (0, 1, 3)) AND (SCADATTPASS = 0)) AS NoteAccr, AnaCRiserv.CODCATEGORIA, 
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[ALD_Vista_UffAcqAnagraficoClientiRiservati] TO [Metodo98]
    AS [dbo];
