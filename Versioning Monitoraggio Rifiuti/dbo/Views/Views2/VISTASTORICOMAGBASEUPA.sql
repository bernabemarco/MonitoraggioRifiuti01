CREATE VIEW VISTASTORICOMAGBASEUPA AS
SELECT STMAG.BIS,
        STMAG.CODART,
	...
	(CASE WHEN TVG.INCLUDISPRIP=0 THEN STMAG.IMPORTOTOTNETTOVAL ELSE STMAG.IMPORTOTOTNETTOVAL+STMAG.SPESERIPMAGVAL END) AS IMPORTOTOTNETTOVAL,
        ...
        STMAG.VARIANTE,
        STMAG.ESERCIZIO,
	(SELECT TOP 1 PREZZOEURO FROM VISTASTORICOPREZZIUM STA WHERE STA.CODARTICOLO=STMAG.CODART AND STA.UMRIC=(SELECT UM FROM ARTICOLIUMPREFERITE AUM WHERE STMAG.CODART=AUM.CODART AND AUM.TIPOUM=1) AND STA.TIPOPREZZO='F' AND STA.DATA <= (SELECT DATAFINEMAG FROM TABESERCIZI WHERE CODICE=STMAG.ESERCIZIO) ORDER BY DATA DESC) AS ULTPREZZOEURO,
	(SELECT TOP 1 PREZZOLIRE FROM VISTASTORICOPREZZIUM STA WHERE STA.CODARTICOLO=STMAG.CODART AND STA.UMRIC=(SELECT UM FROM ARTICOLIUMPREFERITE AUM WHERE STMAG.CODART=AUM.CODART AND AUM.TIPOUM=1) AND STA.TIPOPREZZO='F' AND STA.DATA <= (SELECT DATAFINEMAG FROM TABESERCIZI WHERE CODICE=STMAG.ESERCIZIO) ORDER BY DATA DESC) AS ULTPREZZOLIRE,
	(SELECT TOP 1 UM FROM VISTASTORICOPREZZIUM STA WHERE STA.CODARTICOLO=STMAG.CODART AND STA.UMRIC=(SELECT UM FROM ARTICOLIUMPREFERITE AUM WHERE STMAG.CODART=AUM.CODART AND AUM.TIPOUM=1) AND STA.TIPOPREZZO='F' AND STA.DATA <= (SELECT DATAFINEMAG FROM TABESERCIZI WHERE CODICE=STMAG.ESERCIZIO) ORDER BY DATA DESC) AS UMSTPRZ,
	(SELECT TOP 1 UMRIC FROM VISTASTORICOPREZZIUM STA WHERE STA.CODARTICOLO=STMAG.CODART AND STA.UMRIC=(SELECT UM FROM ARTICOLIUMPREFERITE AUM WHERE STMAG.CODART=AUM.CODART AND AUM.TIPOUM=1) AND STA.TIPOPREZZO='F' AND STA.DATA <= (SELECT DATAFINEMAG FROM TABESERCIZI WHERE CODICE=STMAG.ESERCIZIO) ORDER BY DATA DESC) AS UMRICSTPRZ,
	(SELECT TOP 1 FORMULAFCPRZ FROM VISTASTORICOPREZZIUM STA WHERE STA.CODARTICOLO=STMAG.CODART AND STA.UMRIC=(SELECT UM FROM ARTICOLIUMPREFERITE AUM WHERE STMAG.CODART=AUM.CODART AND AUM.TIPOUM=1) AND STA.TIPOPREZZO='F' AND STA.DATA <= (SELECT DATAFINEMAG FROM TABESERCIZI WHERE CODICE=STMAG.ESERCIZIO) ORDER BY DATA DESC) AS FORMULAFCSTPRZ,
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[VISTASTORICOMAGBASEUPA] TO [Metodo98]
    AS [dbo];
