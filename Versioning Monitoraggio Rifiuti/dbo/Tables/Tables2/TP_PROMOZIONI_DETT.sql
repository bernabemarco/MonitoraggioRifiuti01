CREATE TABLE [dbo].[TP_PROMOZIONI_DETT] (
    ...
    CONSTRAINT [PK__TP_PROMO_DETT] PRIMARY KEY CLUSTERED ([RifProgressivo] ASC, [NrRiga] ASC) WITH (FILLFACTOR = 90)
);


GO

CREATE TRIGGER INSERTTPPROMODETT ON TP_PROMOZIONI_DETT FOR INSERT
AS
...
DECLARE @ATTDIS as smallint
	...
	SELECT @ATTDIS=Attiv_Disat FROM TP_PROMOZIONI_TESTE WHERE Progressivo=@RIFPRG
...
CREATE TRIGGER DELETETPPROMODETT ON TP_PROMOZIONI_DETT FOR DELETE
AS
...
GO
GRANT DELETE
    ON OBJECT::[dbo].[TP_PROMOZIONI_DETT] TO [Metodo98]
    AS [dbo];
