...
-- Data    : 21.04.2016
-- --------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[GRI_INT_TO_MET_RAPPORTI_OreLavoroInterno] (@AZRIF AS VARCHAR(3),@IDRAPPORTO AS VARCHAR(14), @Progressivo AS decimal(10,0)) 
AS
BEGIN
		...
		INSERT INTO GRI_RAPPORTI_OreLavoroInterno
		(
			[IDRAPPORTO]
			,[IDRIGA]
			,[DATA]
		...
		SELECT
			...
			,@DATA_SYNC
		FROM
		GRI_RAPPORTI_OreLavoroInterno MET
		...
		UPDATE MET SET
		 ...
		 ,MET.[KM] = INS.[KM]
		 ,MET.[NumNottiTasferta] = INS.[NumNottiTasferta]
			,MET.[SpeseVittoEVarie] = INS.[SpeseVittoEVarie]
			,MET.[NumeroGGuso] = INS.[NumeroGGuso]
		 ,MET.[UTENTEMODIFICA] = INS.[UTENTEMODIFICA]
		 ,MET.[DATAMODIFICA] = INS.[DATAMODIFICA]
		FROM GRI_RAPPORTI_OreLavoroInterno MET
		INNER JOIN GemmaSync.dbo.GRI_RAPPORTI_OreLavoroInterno INS ON MET.IDRAPPORTO = INS.IDRAPPORTO COLLATE Latin1_General_CI_AS AND MET.IDRIGA = INS.IDRIGA
		WHERE MET.IDRAPPORTO = @IDRAPPORTO 
			AND INS.AZRIF = @AZRIF 
			AND INS.Progressivo = @Progressivo
			AND ISNULL(INS.Eliminato, '0') <> '1'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GRI_INT_TO_MET_RAPPORTI_OreLavoroInterno] TO [Metodo98]
    AS [dbo];
