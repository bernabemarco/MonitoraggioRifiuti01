...
CREATE PROCEDURE [dbo].[GRI_RAPPORTI_FATTURAZIONE_MATERIALI] (@IDRAPPORTO AS VARCHAR(30), @IDSESSIONE AS INT) AS
	/* TEST
...
print @IDRIGA
				...
				IF @DESCINSERITA = 0 BEGIN
					...
					EXEC GRI_PC_INS_ELEMENTIFATT @IDRAPPORTO 
																			...
																			,0						--@idRigaOreLav 
																			,''						--@CodArticolo
																			,'Riepilogo MATERIALI'		--@DscrizioneRiga
																			,0						--@QTA 
																			,0						--@PREZZO 
																			,0						--@SCONTO 
																			,0						--@PROVVIGIONE1 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GRI_RAPPORTI_FATTURAZIONE_MATERIALI] TO [Metodo98]
    AS [dbo];
