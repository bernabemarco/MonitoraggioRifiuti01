...
CREATE FUNCTION [dbo].[FunSCONTI](@Esercizio Int,
              @Fornitore nVarChar(7),
              @DataOdierna datetime,
...
--  GP, Gruppo Prezzi
    ...
    DECLARE ConfigSconti_Cursor CURSOR FOR
        ...
        WHERE 
              TP_CONFIGSCONTI.Attiva = 1 And
               TP_CONFIGSCONTI.CodFor=@Fornitore AND
                TP_CONFIGSCONTI.DaData<=@DataOdierna AND  ((ADATA >= @DataOdierna OR ADATA IS NULL)) AND
                 TP_CONFIGSCONTI.DaData=(SELECT MAX(TP_CONFIGSCONTI.DaData) FROM TP_CONFIGSCONTI WHERE TP_CONFIGSCONTI.CodFor=@Fornitore AND TP_CONFIGSCONTI.DaData<=@DataOdierna AND  ((ADATA >= @DataOdierna OR ADATA IS NULL)))
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FunSCONTI] TO [Metodo98]
    AS [dbo];
