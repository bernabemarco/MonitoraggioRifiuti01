...
CREATE FUNCTION [dbo].[Fun_Vendors_FidoClienti]
(
    ...
    (
        ...
        (
            SELECT DISTINCT
                ...
                ARC.FIDO,
                ARC.FIDOEURO,
                (CASE WHEN ISNULL(ARC.DATAINIZIOCALCOLOFIDO,'') <> '' 
                      THEN ARC.DATAINIZIOCALCOLOFIDO 
        ...
        (
            ...
            (
                /*_NonEmesso*/ 
                SELECT 1 As EscludiFido, Esito, CodCliFor, ImportoScadEuro FROM VistaScadenze WHERE Esito=0 AND TipoEffetto NOT in (SELECT TipoEff FROM ESCLUDIEFF_FIDO WHERE Esercizio=@Esercizio AND Esito=0) And CodCliFor = CLI.CODCONTO AND Convert(varchar,DATAFATTURA,112) >= Convert(varchar,CLI.DATACALCOLO,112)
                UNION ALL
                /*_Emesso*/ 
                SELECT 1 As EscludiFido, Esito, CodCliFor, ImportoScadEuro FROM VistaScadenze WHERE Esito=1 AND TipoEffetto NOT in (SELECT TipoEff FROM ESCLUDIEFF_FIDO WHERE Esercizio=@Esercizio AND Esito=1) And CodCliFor = CLI.CODCONTO AND Convert(varchar,DATAFATTURA,112) >= Convert(varchar,CLI.DATACALCOLO,112)
                UNION ALL
                /*_Pagato*/ 
                SELECT 1 As EscludiFido, Esito, CodCliFor, ImportoScadEuro FROM VistaScadenze WHERE Esito=2 AND TipoEffetto NOT in (SELECT TipoEff FROM ESCLUDIEFF_FIDO WHERE Esercizio=@Esercizio AND Esito=2) And CodCliFor = CLI.CODCONTO  AND Convert(varchar,DATAFATTURA,112) >= Convert(varchar,CLI.DATACALCOLO,112)
                UNION ALL
                /*_Insoluto*/ 
                SELECT 1 As EscludiFido, Esito, CodCliFor, ImportoScadEuro FROM VistaScadenze WHERE Esito=3 AND TipoEffetto NOT in (SELECT TipoEff FROM ESCLUDIEFF_FIDO WHERE Esercizio=@Esercizio AND Esito=3) And CodCliFor = CLI.CODCONTO  AND Convert(varchar,DATAFATTURA,112) >= Convert(varchar,CLI.DATACALCOLO,112)
                UNION ALL
                /*_InsolutoPagato*/ 
                SELECT 1 As EscludiFido, Esito, CodCliFor, ImportoScadEuro FROM VistaScadenze WHERE Esito=4 AND TipoEffetto NOT in (SELECT TipoEff FROM ESCLUDIEFF_FIDO WHERE Esercizio=@Esercizio AND Esito=4) And CodCliFor = CLI.CODCONTO  AND Convert(varchar,DATAFATTURA,112) >= Convert(varchar,CLI.DATACALCOLO,112)
                UNION ALL
                /*ESCLUDI NO*/
                SELECT 0 As EscludiFido, Esito, CodCliFor, ImportoScadEuro FROM VistaScadenze WHERE CodCliFor = CLI.CODCONTO AND Convert(varchar,DATAFATTURA,112) >= Convert(varchar,CLI.DATACALCOLO,112)
            ) t1
            PIVOT
            (
            ...
            (
                ...
                (
                    ...
                    WHERE 
                        ...
                        TCAM.Codice=0 AND DocChiuso=0 AND ((SCA.USACLIFATT = 0 AND CODCLIFOR = SCA.CODCONTO) OR (SCA.USACLIFATT = 1 AND CODCFFATT = SCA.CODCONTO)) AND Convert(varchar,DataDoc,112)>=Convert(varchar,SCA.DATACALCOLO,112) AND 
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[Fun_Vendors_FidoClienti] TO [Metodo98]
    AS [dbo];
