...
CREATE FUNCTION [dbo].[FunDataDocOrigine](@ProgressivoRP Decimal(10,0),@IdRigaRP INT)
RETURNS Varchar(8)
AS
BEGIN

DECLARE @DataOrdine varchar(8)
	
	SET @DataOrdine = CONVERT(varchar(8),GetDate(),112);
    ...
    AS ( SELECT RD.TIPODOC AS Doc ,
                ...
                RD.IDRIGARP ,
                0 AS level,
                CONVERT(varchar(8),TD.DATADOC,112) as DATADOC,
                TD.NUMRIFDOC,
                TD.DATARIFDOC
        ...
        SELECT  cte.TIPODOC AS Doc ,
                ...
                R2.IDRIGARP ,
                cte.level + 1,
                CONVERT(varchar(8),T2.DATADOC,112) as DATADOC,
                T2.NUMRIFDOC,
                T2.DATARIFDOC
    ...
    SELECT @DataOrdine = DATADOC FROM cte INNER JOIN (select max(level) as level from cte) cte_g on cte_g.level = cte.level
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FunDataDocOrigine] TO [Metodo98]
    AS [dbo];
