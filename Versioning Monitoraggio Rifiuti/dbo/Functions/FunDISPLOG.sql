...
CREATE FUNCTION FunDISPLOG(@ARTICOLO nVARCHAR(50),@DEPOSITO nVARCHAR(10),@NRIFPARTITA nVARCHAR(15))
...
DECLARE @DISP          	DECIMAL(19,6)



	DECLARE Articoli_Cursor CURSOR fast_forward read_only  FOR
	
	SELECT TAB.CODUBICAZIONE,TAB.TIPOUBICAZIONE,SUM(TAB.GIAC + TAB.ORDINATO - TAB.IMPEGNATO) AS DISP  FROM
             (SELECT
		 ...
		 SUM(CASE WHEN GIACENZA=-1 and RESO = 1 THEN QTA1UM ELSE 0 END)) AS GIAC ,
                            SUM(QTA1UM*ORDINATO)    AS ORDINATO,
                            SUM(QTA1UM*IMPEGNATO) AS IMPEGNATO   
	...
	FETCH NEXT FROM Articoli_Cursor
 	    INTO @CODUBICAZIONE,@TIPOUBICAZIONE,@DISP
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
			IF @DISP > 0
                                             ...
                                             BEGIN                                        
                                                    INSERT INTO @Results VALUES(@CODUBICAZIONE,@TIPOUBICAZIONE,@DISP)
                                             END 
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[FunDISPLOG] TO [Metodo98]
    AS [dbo];
