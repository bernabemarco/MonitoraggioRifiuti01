...
CREATE FUNCTION [Price].[GetRowsTablet] ( @Rows As Progressivi_GestionePrezzi READONLY , @Type bit)
...
BEGIN
    ...
    SELECT CODCONTO , LISTINO, USAPRZCOMMERCIALI FROM AnagraficaClientitablet;
    ...
    FROM dbo.REGOLEPREZZIESTESI GP 
        ...
        INNER JOIN dbo.TP_EXTRAMAG TEM On  TEM.CODART like GP.CODARTTIPOLOGIA
    ...
    WHERE   USAPRZCOMMERCIALI=1 AND (CODGRUPPOPREZZIMAG = 0) 
        ...
        AND (TEM.ESPORTABILE = 0 AND  TEM.EscludiVal=0) 
        AND @type =1
    ...
    FROM dbo.REGOLEPREZZIESTESI GP 
        ...
        INNER JOIN dbo.TP_EXTRAMAG TEM On  TEM.CODART like ART.CODICE
    ...
    FROM  REGOLEPREZZIESTESI GP
            ...
            INNER JOIN dbo.TP_EXTRAMAG TEM On  TEM.CODART = ART.CODICEART
            LEFT OUTER JOIN @CliFor CLIFOR ON  CLIFOR.CODCONTO =GP.CODCLIFOR 
            LEFT OUTER JOIN dbo.ARTICOLIUMPREFERITE UMP ON UMP.CODART = TEM.CODART AND UMP.TIPOUM =3
            LEFT OUTER JOIN (SELECT 0 CODICE,CODCONTO FROM dbo.ANAGRAFICACF 
                            UNION     
    ...
    WHERE   C.USAPRZCOMMERCIALI=1
            ...
            AND GP.FINEVALIDITA >= CONVERT(DATE, GETDATE())
            AND (TEM.ESPORTABILE = 0 OR TEM.ESPORTABILE IS NULL)
            AND @type =1
    ...
    FROM dbo.GESTIONEPREZZI GP 
        ...
        INNER JOIN dbo.TP_EXTRAMAG TEM On  TEM.CODART like GP.CODARTRIC
        LEFT OUTER JOIN dbo.ARTICOLIUMPREFERITE UMP ON UMP.CODART = TEM.CODART AND UMP.TIPOUM =3
