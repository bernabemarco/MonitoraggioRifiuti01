...
-- Data    : 21.04.2016
-- --------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[GRI_INT_TO_MET_RAPPORTI_ServiziAccessori] (@AZRIF AS VARCHAR(3), @IDRAPPORTO AS VARCHAR(14), @Progressivo AS decimal(10,0))
AS
BEGIN
	...
	INSERT INTO GRI_RAPPORTI_ServiziAccessori
	(
		[IDRAPPORTO]
		,[IDRIGA]
		,[IDMATERIALE]
	...
	SELECT
		...
		,INS.[IDRIGA]
		,INS.[IDMATERIALE]	COLLATE Latin1_General_CI_AS
		,'Sincronizzazione'
		,@DATA_SYNC
	FROM GemmaSync.dbo.GRI_RAPPORTI_ServiziAccessori INS
	WHERE ISNULL(INS.ELIMINATO, '0') = '0' AND INS.IDRAPPORTO = @IDRAPPORTO COLLATE Latin1_General_CI_AS AND INS.AZRIF = @AZRIF and INS.Progressivo = @Progressivo
	EXCEPT
	SELECT
		MET.[IDRAPPORTO]
		,MET.[IDRIGA]
		...
		,'Sincronizzazione'
		,@DATA_SYNC
	FROM GRI_RAPPORTI_ServiziAccessori MET
	...
	UPDATE MET SET
		...
		FROM GRI_RAPPORTI_ServiziAccessori MET
		INNER JOIN GemmaSync.dbo.GRI_RAPPORTI_ServiziAccessori INS ON MET.IDRAPPORTO = INS.IDRAPPORTO COLLATE Latin1_General_CI_AS AND MET.IDRIGA = INS.IDRIGA
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GRI_INT_TO_MET_RAPPORTI_ServiziAccessori] TO [Metodo98]
    AS [dbo];
