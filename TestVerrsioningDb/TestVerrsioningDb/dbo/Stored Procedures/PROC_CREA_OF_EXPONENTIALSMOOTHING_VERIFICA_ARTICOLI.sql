...
CREATE PROCEDURE PROC_CREA_OF_EXPONENTIALSMOOTHING_VERIFICA_ARTICOLI
--ENCRY--
AS
...
DECLARE @TRENDPREVISTO      DECIMAL(19,6)   
    ...
    WHILE @@FETCH_STATUS = 0
        BEGIN
            SET @SQLQuery = N'IF NOT EXISTS(SELECT CODARTICOLO FROM TP_STAGIONALE_PREVISIONI WHERE CODARTICOLO = ''' + @CODARTICOLO + ''' AND CODDEPOSITO = ''' + @CODDEPOSITO + ''' AND TIPO = ''SE'')
                              INSERT INTO TP_STAGIONALE_PREVISIONI(CodArticolo,CodDeposito,Tipo,UtenteModifica,DataModifica)
                              SELECT ''' + @CODARTICOLO + ''' , ''' + @CODDEPOSITO + ''' ,''SE'', USER_NAME(), GETDATE()'

            SET @SQLQuery = @SQLQuery + N';IF NOT EXISTS(SELECT CODARTICOLO FROM TP_STORICO_EXPONENTIALSMOOTHING WHERE CODARTICOLO = ''' + @CODARTICOLO + ''' AND CODDEPOSITO = ''' + @CODDEPOSITO + ''')
                                           INSERT INTO TP_STORICO_EXPONENTIALSMOOTHING(CodArticolo,CodDeposito,UtenteModifica,DataModifica)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[PROC_CREA_OF_EXPONENTIALSMOOTHING_VERIFICA_ARTICOLI] TO [Metodo98]
    AS [dbo];
