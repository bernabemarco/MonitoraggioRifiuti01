...
-- --------------------------------------------------------------------------------------
-- Funzione  : GRI_RAPPORTIMEZZI (Metodo->DbSync)
-- Autore   : Moreno Feletto
-- Data    : 20.04.2016
-- --------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[GRI_METSIC_TO_INT_RAPPORTI_Mezzi] (@IDRAPPORTO AS VARCHAR(14),@TIPOOPERAZIONE AS char(1),@Progressivo AS decimal(10,0),@Stato AS VARCHAR(1)) 
AS
BEGIN
...
IF @Stato = 'T'
	BEGIN
		...
		UPDATE INS SET
			...
			INNER JOIN GRI_RAPPORTIMEZZI MET ON INS.IDRAPPORTO = MET.IDRAPPORTO COLLATE Latin1_General_CI_AS AND INS.IDRIGAMEZZO = MET.IDRIGAMEZZO AND INS.AZRIF = @IdAzienda COLLATE Latin1_General_CI_AS 
...
ELSE
	BEGIN
		...
		INSERT INTO GemmaSync.dbo.GRI_RAPPORTIMEZZI
		(
			[IDRAPPORTO]
			,[AZRIF]
			,[PROGRESSIVO]
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GRI_METSIC_TO_INT_RAPPORTI_Mezzi] TO [Metodo98]
    AS [dbo];
