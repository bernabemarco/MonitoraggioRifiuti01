...
CREATE PROCEDURE [dbo].[PROC_CREA_OF_STATISTICHE_STAGIONALI](@ALL_STATISTICS SMALLINT)
AS
...
DECLARE @SqlQuery           AS nVARCHAR(MAX)
    ...
    /*VENDUTO*/
    INSERT INTO TP_STAGIONALE_VENDUTO(CODARTICOLO,CODDEPOSITO,PERIODOOSSERVAZIONE,UTENTEMODIFICA,DATAMODIFICA)
    SELECT
        VENDUTI.CODARTICOLO,
        VENDUTI.CODDEPOSITO,
    ...
    /*VENDUTO MEDIA*/
    INSERT INTO TP_STAGIONALE_VENDUTO_MEDIA(CODARTICOLO,CODDEPOSITO,PERIODOOSSERVAZIONE,UTENTEMODIFICA,DATAMODIFICA)
    SELECT
        VENDUTI.CODARTICOLO,
        VENDUTI.CODDEPOSITO,
        VENDUTI.OSSERVAZIONE,
    ...
    /*ACQUISTATO*/
    INSERT INTO TP_STAGIONALE_ACQUISTATO(CODARTICOLO,CODDEPOSITO,PERIODOOSSERVAZIONE,UTENTEMODIFICA,DATAMODIFICA)
    SELECT
    ...
    /*ORDINATO*/
    INSERT INTO TP_STAGIONALE_ORDINATO(CODARTICOLO,CODDEPOSITO,PERIODOOSSERVAZIONE,UTENTEMODIFICA,DATAMODIFICA)
    SELECT
    ...
    OPEN PERIODI_CUR    
        ...
        WHILE @@FETCH_STATUS = 0
            BEGIN
                ...
                SELECT @PERIODI_CAMBIATI = COUNT(Periodo) FROM TP_STAGIONALE_CALCOLATI WHERE OSSERVAZIONE = @OSSERVAZIONE AND CAMBIATO = 1
                    ...
                    BEGIN
                        ...
                        WHILE (@PERIODI_CORRENTE > 0)
                            BEGIN
                                ...
                                /*VENDUTI*/
                                SET @SqlQuery = N'UPDATE TP_STAGIONALE_VENDUTO SET [' + CAST(@PERIODI_TOTALE AS nVARCHAR(2)) + '] = [' + CAST(@PERIODI_CORRENTE AS nVARCHAR(2)) + ']
                                                  WHERE PERIODOOSSERVAZIONE = ' + CAST(@OSSERVAZIONE AS nVARCHAR(2))
                                
                                exec sp_executesql @SqlQuery
                                
                                /*VENDUTI MEDIA*/
                                SET @SqlQuery = N'UPDATE TP_STAGIONALE_VENDUTO_MEDIA SET [' + CAST(@PERIODI_TOTALE AS nVARCHAR(2)) + '] = [' + CAST(@PERIODI_CORRENTE AS nVARCHAR(2)) + ']
                                                  WHERE PERIODOOSSERVAZIONE = ' + CAST(@OSSERVAZIONE AS nVARCHAR(2))
                                
                                exec sp_executesql @SqlQuery
                                
                                /*ACQUISTATI*/
                                SET @SqlQuery = N'UPDATE TP_STAGIONALE_ACQUISTATO SET [' + CAST(@PERIODI_TOTALE AS VARCHAR(2)) + '] = [' + CAST(@PERIODI_CORRENTE AS VARCHAR(2)) + '] 
                                                  WHERE PERIODOOSSERVAZIONE = ' + CAST(@OSSERVAZIONE AS VARCHAR(2))
                                
                                exec sp_executesql @SqlQuery
                                
                                /*ORDINATO*/
                                SET @SqlQuery = N'UPDATE TP_STAGIONALE_ORDINATO SET [' + CAST(@PERIODI_TOTALE AS VARCHAR(2)) + '] = [' + CAST(@PERIODI_CORRENTE AS VARCHAR(2)) + '] 
                                                  WHERE PERIODOOSSERVAZIONE = ' + CAST(@OSSERVAZIONE AS VARCHAR(2))
                        ...
                        WHILE (@PERIODI_CORRENTE <= @PERIODI_CAMBIATI)
                            BEGIN
                                ...
                                EXEC PROC_CALCOLA_OF_STAGIONALEVENDUTO @PERIODI_CORRENTE, @OSSERVAZIONE, @VENDOC, @VENCAU, @ALL_STATISTICS
                                EXEC PROC_CALCOLA_OF_STAGIONALEVENDUTOMEDIA @PERIODI_CORRENTE, @OSSERVAZIONE, @VENDOC, @VENCAU, @ALL_STATISTICS
                                EXEC PROC_CALCOLA_OF_STAGIONALEACQUISTATO @PERIODI_CORRENTE, @OSSERVAZIONE, @ACQDOC, @ACQCAU, @ALL_STATISTICS
                                EXEC PROC_CALCOLA_OF_STAGIONALEORDINATO @PERIODI_CORRENTE, @OSSERVAZIONE, @ALL_STATISTICS
                ...
                EXEC PROC_CALCOLA_OF_STAGIONALEVENDUTO 1, @OSSERVAZIONE, @VENDOC, @VENCAU, @ALL_STATISTICS
                EXEC PROC_CALCOLA_OF_STAGIONALEVENDUTOMEDIA 1, @OSSERVAZIONE, @VENDOC, @VENCAU, @ALL_STATISTICS
                EXEC PROC_CALCOLA_OF_STAGIONALEACQUISTATO 1, @OSSERVAZIONE, @ACQDOC, @ACQCAU, @ALL_STATISTICS
                EXEC PROC_CALCOLA_OF_STAGIONALEORDINATO 1, @OSSERVAZIONE, @ALL_STATISTICS
    
                EXEC PROC_CALCOLA_OF_STAGIONALEVENDUTO 2, @OSSERVAZIONE, @VENDOC, @VENCAU, @ALL_STATISTICS
                EXEC PROC_CALCOLA_OF_STAGIONALEVENDUTOMEDIA 2, @OSSERVAZIONE, @VENDOC, @VENCAU, @ALL_STATISTICS
                EXEC PROC_CALCOLA_OF_STAGIONALEACQUISTATO 2, @OSSERVAZIONE, @ACQDOC, @ACQCAU, @ALL_STATISTICS
                EXEC PROC_CALCOLA_OF_STAGIONALEORDINATO 2, @OSSERVAZIONE, @ALL_STATISTICS
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[PROC_CREA_OF_STATISTICHE_STAGIONALI] TO [Metodo98]
    AS [dbo];
