...
CREATE PROCEDURE [dbo].[PROC_CALCOLA_OF_STAGIONALEVENDUTOMEDIA](@PERIODO INT,
                                                                 @OSSERVAZIONE INT,
...
DECLARE @SQL nVARCHAR(4000)
    ...
    SET @SQL = N'UPDATE
                    TP_STAGIONALE_VENDUTO_MEDIA
                 SET
                    TP_STAGIONALE_VENDUTO_MEDIA.[' + CAST(@PERIODO AS VARCHAR(2)) + '] = ISNULL(ARTICOLI.SUMQTA,0)
                 FROM
                    TP_STAGIONALE_VENDUTO_MEDIA TSV
                 LEFT JOIN
                    (
                        SELECT 
                            TEM.CODART,
                            STMAG.CODDEPOSITO,
                            (CASE WHEN TSC.GIORNIPERIODO > 0 THEN (SUM(STMAG.QTA1UM * (STMAG.GIACENZA * -1)) / TSC.GIORNIPERIODO) ELSE 0 END) AS SUMQTA
                         FROM 
                            STORICOMAG STMAG, 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[PROC_CALCOLA_OF_STAGIONALEVENDUTOMEDIA] TO [Metodo98]
    AS [dbo];
