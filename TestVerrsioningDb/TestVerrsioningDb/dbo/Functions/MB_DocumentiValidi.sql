...
CREATE FUNCTION [dbo].[MB_DocumentiValidi] (@IDSessione DECIMAL(5,0))
    RETURNS TABLE 
AS
RETURN 
(
        ...
        (       
            ...
            (
                ...
                FROM
                    TESTEDOCUMENTI T
                INNER JOIN
                    (SELECT DISTINCT IDTESTA FROM MB_RIFRIGHEDOCUMENTI_TMP WHERE IDSESSIONE = @IDSessione) TT
                ON
                    T.PROGRESSIVO = TT.IDTESTA
                LEFT JOIN
    ...
    INNER JOIN 
        RIGHEDOCUMENTI AS R ON R.IDTESTA = T.PROGRESSIVO
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[MB_DocumentiValidi] TO [Metodo98]
    AS [dbo];
