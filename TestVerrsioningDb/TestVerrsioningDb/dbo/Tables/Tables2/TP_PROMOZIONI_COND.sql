CREATE TABLE [dbo].[TP_PROMOZIONI_COND] (
    [RifProgressivo]  DECIMAL (18)    NOT NULL,
    [NrRiga]          INT             NOT NULL,
    [FlagRiga]        SMALLINT        NULL,
    [Posizione]       INT             NULL,
    [Codice]          VARCHAR (50)    NULL,
    [UM_M]            VARCHAR (3)     NULL,
    [Qta_Minima]      NUMERIC (16, 6) NULL,
    [Note]            VARCHAR (2500)  NULL,
    [UTENTEMODIFICA]  VARCHAR (25)    NULL,
    [DATAMODIFICA]    DATETIME        NULL,
    [QtaBudget]       DECIMAL (19, 6) NULL,
    [ValoreBudget]    DECIMAL (19, 6) NULL,
    [DataConsuntiv]   DATETIME        NULL,
    [QtaConsuntiv]    DECIMAL (19, 6) NULL,
    [ValoreConsuntiv] DECIMAL (19, 6) NULL,
    CONSTRAINT [PK__TP_PROMO_COND] PRIMARY KEY CLUSTERED ([RifProgressivo] ASC, [NrRiga] ASC) WITH (FILLFACTOR = 90)
);


GO

CREATE TRIGGER INSERTTPPROMOCOND ON TP_PROMOZIONI_COND FOR INSERT
AS

BEGIN

DECLARE @RIFPRG as int
DECLARE @CODART as Varchar(50)
DECLARE @ATTDIS as smallint
	
	SELECT @RIFPRG=RifProgressivo,  @CODART=Codice FROM INSERTED 
	SELECT @ATTDIS=Attiv_Disat FROM TP_PROMOZIONI_TESTE WHERE Progressivo=@RIFPRG
	
	IF @ATTDIS=0
		BEGIN
			UPDATE TP_EXTRAMAG SET TP_Promo_com=@RIFPRG WHERE CODART=@CODART
		END
END



GO

CREATE TRIGGER DELETETPPROMOCOND ON TP_PROMOZIONI_COND FOR DELETE
AS

BEGIN

DECLARE @CODART as VarChar(50)
	
	SELECT @CODART=Codice FROM DELETED 

	UPDATE TP_EXTRAMAG SET TP_Promo_com='' WHERE CODART=@CODART

END



GO
GRANT DELETE
    ON OBJECT::[dbo].[TP_PROMOZIONI_COND] TO [Metodo98]
    AS [dbo];


GO
GRANT INSERT
    ON OBJECT::[dbo].[TP_PROMOZIONI_COND] TO [Metodo98]
    AS [dbo];


GO
GRANT REFERENCES
    ON OBJECT::[dbo].[TP_PROMOZIONI_COND] TO [Metodo98]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[TP_PROMOZIONI_COND] TO [Metodo98]
    AS [dbo];


GO
GRANT UPDATE
    ON OBJECT::[dbo].[TP_PROMOZIONI_COND] TO [Metodo98]
    AS [dbo];

