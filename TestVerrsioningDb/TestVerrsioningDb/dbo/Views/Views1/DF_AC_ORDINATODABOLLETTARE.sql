...
CREATE VIEW DF_AC_ORDINATODABOLLETTARE
AS
...
SELECT T.[PROGRESSIVO]
  ...
  FROM [PARAMETRIDOC] P
  INNER JOIN [TESTEDOCUMENTI] T ON P.TIPO IN('O') AND P.CLIFOR = 'C' AND P.CODICE = T.TIPODOC AND T.DOCCHIUSO = 0
...
GO
GRANT SELECT
    ON OBJECT::[dbo].[DF_AC_ORDINATODABOLLETTARE] TO [Metodo98]
    AS [dbo];
