CREATE VIEW BudVistaEliminaBudget AS 
    ...
    WHERE Ambiente IN (SELECT Codice FROM BudAmbientiBudget WHERE Tipo = 2 OR Tipo = 3)
    GROUP BY Stagione, AnnoStagione, TipologiaProdotto, Ambiente

GO
GRANT SELECT
    ON OBJECT::[dbo].[BudVistaEliminaBudget] TO [Metodo98]
    AS [dbo];
