...
SELECT 
    ...
    NULL AS DATAREG, 
    (CASE WHEN RCONT.SEGNO=1 THEN RCONT.IMPORTO ELSE 0 END)-(CASE WHEN RCONT.SEGNO=0 THEN RCONT.IMPORTO ELSE 0 END) AS SALDO,
    (CASE WHEN RCONT.SEGNO=1 THEN RCONT.IMPORTOEURO ELSE 0 END)-(CASE WHEN RCONT.SEGNO=0 THEN RCONT.IMPORTOEURO ELSE 0 END) AS SALDOEURO,
    (CASE WHEN RCONT.SEGNO=1 THEN RCONT.IMPORTOVALUTA ELSE 0 END)-(CASE WHEN RCONT.SEGNO=0 THEN RCONT.IMPORTOVALUTA ELSE 0 END) AS SALDOVALUTA,
  ...
  WHERE (TCONT.CAUSALE=(SELECT TOP 1 GIC.CAUSCONTCH FROM TABVINCOLIGIC AS GIC WHERE GIC.ESERCIZIO=RCONT.ESERCIZIO) OR TCONT.CAUSALE IS NULL)
  AND TABCFG.TIPOMASTRO<>'E'


GO
GRANT SELECT
    ON OBJECT::[dbo].[VISTABILANCI2] TO [Metodo98]
    AS [dbo];
