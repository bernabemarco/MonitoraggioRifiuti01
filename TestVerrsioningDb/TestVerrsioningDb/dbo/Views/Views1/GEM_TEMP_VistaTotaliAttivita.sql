...
/****** Oggetto:  View [dbo].[GEM_TEMP_VistaTotaliAttivita]    Data script: 04/03/2013 14:32:38 ******/

CREATE VIEW [dbo].[GEM_TEMP_VistaTotaliAttivita]
AS
SELECT     SC_RDM.UTENTEMODIFICA_stp, SC_RDM.IDRAPPORTO, TC.IDCONTRATTO, TC.CODCLIENTE, SC.SEZIONECONTRATTO, SC.DESCRIZIONESEZIONE, SC.CODDEST, SC.idTipologMezzo, 
...
FROM         GEM_TEMP_SEZIONECONTRATTORAPPORTIMEZZI AS SC_RDM LEFT OUTER JOIN
...
GROUP BY SC_RDM.UTENTEMODIFICA_stp, TC.IDCONTRATTO, TC.CODCLIENTE, SC.SEZIONECONTRATTO, SC.DESCRIZIONESEZIONE, SC.CODDEST, SC.idTipologMezzo, SC.CodiceMezzo, 
                      SC_RDM.CodMezzo, SC.REPARTO, AnaArt.DESCRIZIONE, SC_RDM.IDRAPPORTO, SC_RDM.StatoAttivita

GO
GRANT SELECT
    ON OBJECT::[dbo].[GEM_TEMP_VistaTotaliAttivita] TO [Metodo98]
    AS [dbo];
