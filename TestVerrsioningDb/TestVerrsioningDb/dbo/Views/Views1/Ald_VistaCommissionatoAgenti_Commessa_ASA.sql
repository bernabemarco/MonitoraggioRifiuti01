...
CREATE VIEW [dbo].[Ald_VistaCommissionatoAgenti_Commessa_ASA]
as
SELECT     TD.CODAGENTE1, TD.CODAGENTE2, TD.CODAGENTE3, CONS_ALD_TABASA.IdPrestazione, RifCommAsa.IdAsaCommessa, RD.RIFCOMMCLI AS RifComm, 
                      AC.CODMASTRO AS MastroCliente, TM.idOrFatt, RD.GENCONTROP, TD.CODCFFATT AS CODCLIFOR, ExC.CodSap, 
					  AC.PROVINCIA, AC.LOCALITA, ExC.ClassCID, 
                      ExC.IdTrust, TD.ESERCIZIO, format(MONTH(TD.DATADOC), '00') AS MESE, TM_Generici.TIPO AS NaturaContoGenerico,
					   ExTD.IdDivisione, TD.TIPODOC, td.NUMERODOC,
                      TD.CODPAGAMENTO, 
					  ...
					  YEAR(RD.DATACONSEGNA) AS AnnoConsegna, format(MONTH(RD.DATACONSEGNA), '00') AS MeseConsegna, 
					  TD.BLOCCATO,
                      SUM(RD.TOTNETTORIGARES * TD.SEGNO) AS ValCommisionatoEuroRes
FROM         CONS_ALD_TABASA RIGHT OUTER JOIN
                      ...
                      CONS_ALD_TabTipoDocCommissionato AS TabTipiDoc ON TD.TIPODOC = TabTipiDoc.TipoDoc LEFT OUTER JOIN
...
WHERE     (AC.TIPOCONTO = 'C') AND (TD.DOCCHIUSO = 0) AND (RD.TOTNETTORIGARES > 0)
GROUP BY TD.CODAGENTE1, TD.CODAGENTE2, TD.CODAGENTE3, RifCommAsa.IdAsaCommessa, RD.RIFCOMMCLI, AC.CODMASTRO, TM.idOrFatt, RD.GENCONTROP, 
                      TD.CODCFFATT, AC.PROVINCIA, AC.LOCALITA, ExC.ClassCID, ExC.IdTrust, 
					  TD.ESERCIZIO, format(MONTH(TD.DATADOC), '00'), TM_Generici.TIPO, ExTD.IdDivisione, 
                      TD.TIPODOC, td.NUMERODOC, TD.CODPAGAMENTO, YEAR(RD.DATACONSEGNA), 
					  format(MONTH(RD.DATACONSEGNA), '00'), CONS_ALD_TABASA.IdPrestazione, ExC.CodSap, TD.BLOCCATO
...
GO
GRANT DELETE
    ON OBJECT::[dbo].[Ald_VistaCommissionatoAgenti_Commessa_ASA] TO [Metodo98]
    AS [dbo];
