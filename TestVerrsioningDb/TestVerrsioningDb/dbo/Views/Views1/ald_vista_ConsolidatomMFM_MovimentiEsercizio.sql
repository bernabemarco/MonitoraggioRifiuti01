...
CREATE VIEW [dbo].[ald_vista_ConsolidatomMFM_MovimentiEsercizio]
AS
SELECT     DB_NAME() AS Ditta, RIGHECONTABILITA.ESERCIZIO, 'ME' as Natura,RIGHECONTABILITA.DATAREG, RIGHECONTABILITA.CONTO, TABCFG.DSCCONTO1, 
...
and testecontabilita.causale not in (select CAUSCONTAP from TABVINCOLIGIC where TABVINCOLIGIC.ESERCIZIO=testecontabilita.Esercizio)
and testecontabilita.causale not in (select CAUSCONTCH from TABVINCOLIGIC where TABVINCOLIGIC.ESERCIZIO=testecontabilita.Esercizio)

GO
GRANT DELETE
    ON OBJECT::[dbo].[ald_vista_ConsolidatomMFM_MovimentiEsercizio] TO [Metodo98]
    AS [dbo];
